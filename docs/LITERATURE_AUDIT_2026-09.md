...
| 链 C 当前动作→未来可行空间 | ★★★★★（一号选择） | **技术立足点已找到**（约束型 ≠ 惩罚型，具固定翼特异性）；**新颖性仍待第二轮审计**（概念可能已在 reachability/viability 名下存在） | 第 3 节 |
...
**链 C 不存在初版所称的"致命反驳"**（那基于一个已被撤回的错误断言）。

> **更正声明（2026-09-17）**：本节初版的断言 1 是**错的**，已撤回。
> 更正与证据见 `notes/CHAIN_C_THEORY_NOTE.md` 与 `notes/verify_chainC_theory.py`。
> 结论摘要：势函数差分形式 `γΦ(s')−Φ(s)` 才保持策略不变；
> 若 `F_future` 依赖**结果**状态 `s_{t+1}`（即 `a_t → F_{t+1}`），
> 它**会**改变最优策略，因此按此形式设计是**安全**的。
> 真正可用的是本节的 **3.1b**（约束型 ≠ 惩罚型，固定翼特异性）。

### 3.1 奖励重塑的策略不变性（范围被初版高估）
...
- 若 `F_future` 是**当前**状态的函数，它对所有候选动作加同一个常数而不改变 argmax，
  **但也因此无法区分动作——作为决策指标完全无效**。
- 若 `F_future` 依赖**结果**状态 `s_{t+1}`（形式 `a_t → F_{t+1}`），
  它是一般的状态奖励，**会**改变最优策略。**这正是桌面文档主张的形式，因而是安全的。**
- 仅当 `F_future` 恰可写成 `γΦ(s') − Φ(s)` 时，最优策略才不变。
  这是一种可能被审稿人提出的等价化攻击，需在论文中主动排除，但**不是项目的实际风险**。

### 3.1b 真正可用的判据：约束型 ≠ 惩罚型（固定翼特异性）

见 `notes/CHAIN_C_THEORY_NOTE.md`。核验结果：

| 算例 | 约束最优 `argmin J s.t. F ≥ F_min` 是否为支撑点 |
| --- | --- |
| 固定翼式**离散热机动集**（非凸权衡） | **非支撑**——无任何 λ ≥ 0 可复现该点 |
| 类多旋翼**凸权衡集** | 支撑——λ ∈ (1.19, 1.21) 即可复现 |

含义：当机动集非凸时，"按未来可行空间约束选动作"**在数学上不等于**
"给代价加一项惩罚"，二者结论可以不同；当权衡集凸时二者等价。
固定翼的机动集（Dubins 离散词表、最小转弯半径、风场下可达集的非凸性）
天然非凸，多旋翼近似凸。**这是"换成四旋翼就不成立"的技术挡板。**

**边界**：该结果来自构造的有限算例，非一般性定理。
要立项需补：(i) 固定翼机动集非凸性的结构性证明；(ii) 或"约束最优非支撑"的充分条件。

### 3.2 由此推出的实验硬约束（比初版的"逃逸路径"更可操作）

由 3.1b 直接得到：

> **若实验场景使权衡集近似凸，约束型与惩罚型会给出相同结果，
> 实验必然测出"future-aware 无改善"，项目得到无法解读的假阴性。**

必须在预注册中先做可行性检查：
(i) 采用离散机动词表（Dubins 路径族的有限子集）；
(ii) 或构造使可达集非凸的场景（狭窄通道、风场造成的可达域凹口）；
(iii) 先验证该场景下约束最优点确实是非支撑的，再比较决策规则。

否则零结果无法区分"机制不存在"与"算例太滑"。

相关的可借鉴传统（非马尔可夫奖励的决策论规划）：
| 标题 | 年份 | 期刊 | 第一作者 | DOI |
| --- | --- | --- | --- | --- |
...
链 C **不存在初版所称的"致命反驳"**——那条反驳基于一个错误断言，已撤回。

修正后的判定：

- 按 `a_t → F_{t+1}`（依赖结果状态）的形式设计，**决策上是有效的**；
  按 `J_now − λ·F_future` 的惩罚形式做，也**能**产生非平凡结论。
- 但惩罚形式会招致"这不就是奖励工程"的质疑，且存在被 3.1 的等价化攻击的空间。
- **推荐采用约束形式** `argmin J s.t. F(a) ≥ F_min`：3.1b 给出了它在固定翼机动集
  非凸时不可归约为惩罚项的判据，这是本项目最硬的技术立足点。
- **立项前必须完成**：(i) 验证所选场景的机动集确实非凸（3.2 的可行性检查）；
  (ii) 补固定翼机动集非凸性的文献证据或证明。
- **仍未解决的老问题**（初版第 3.3 节，不受本次更正影响）：
  "未来可行空间/选项保留"不是本领域既有术语，该概念可能在
  reachability / viability / safety margin / backup plan / abort feasibility
  等名字下已被处理。**必须用第二轮审计查清，否则新颖性无法主张。**
...
1. **链 C 若要继续**，分两步走，顺序不能颠倒：
   (a) **新颖性审计（第二轮）**：查清"未来可行空间/选项保留"是否已在
       reachability / viability kernel / safety margin / backup plan /
       abort feasibility 等既有名称下被处理。**不查清就不能主张新颖性。**
   (b) **技术立足点已完成**：约束型 ≠ 惩罚型的判据见
       `notes/CHAIN_C_THEORY_NOTE.md`（固定翼离散热机动集下约束最优点非支撑）。
       接着需补固定翼机动集非凸性的结构性论证，并在预注册里做 3.2 的可行性检查。
...
- **第 3 节初版含一处错误断言（已在 2026-09-17 撤回并更正）**：
  初版称"F_future 若为当前状态的函数则最优策略不变"，
  该断言不成立（势函数差分形式才保持策略不变）。更正见
  `notes/CHAIN_C_THEORY_NOTE.md` 与 `notes/verify_chainC_theory.py`。
  本条按 `CONVENTIONS.md` 第 5 节"失败保留"要求留档。
