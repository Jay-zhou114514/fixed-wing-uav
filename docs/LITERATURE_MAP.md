...
### 3.7.1 独立复核后的加强（2026-09-18）

应负责人要求做了独立复核（`experiments/EXP-FW-V1-27A-independent-recheck.md`），
结论**比 3.7 节更不利**：
...
| "问题发现"重叠 | 是 | 是 |
| **"机制"重叠** | 未判 | **是**（转弯圆放不下） |
| **"判据形式"重叠** | 未判 | **是**（该文用**同时**判据：2 个转弯圆 × 3 条边全评估，选未越界者） |
| 可主张增量 | 形式化 + 量化 | **仅剩穿透量闭式公式 `R·cos(θ_int/2)`** + 形式化 + 对 JAIS 逐边判据的否证 |

**检索证据**：OpenAlex 全库 `geofence + acute + vertex + turn radius + penetration`
**仅命中 Thomas 2024 一篇**，说明该交叉点在文献中稀疏，但**已有覆盖**。

**因此**：本项发现应**降级为平台层的几何前置条件说明**，
**不得**作为论文的独立贡献；论文贡献重心维持 C1+C2+C4。
